...
import { MapPin, Anchor, Palmtree, Car, Camera, Music, Utensils, Mountain } from 'lucide-react';
...
const ExperiencesSection = ({ formData, updateFormData }) => {
  ...
  const getColorClasses = (color, isSelected) => {
    const colors = {
      ...
      orange: isSelected ? 'border-orange-400 bg-orange-50' : 'border-gray-200 hover:border-orange-300',
      ...
      indigo: isSelected ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-indigo-300'
    };
  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl font-bold text-gray-800 mb-2">Additional Experiences</h2>
        <p className="text-gray-600">Make your wedding celebration unforgettable</p>
      </div>
      ...
      >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {experiences.map((experience, index) => {
            ...
            const isSelected = formData.additionalExperiences?.includes(experience.value);
            
            return (
              <motion.button
                key={experience.value}
                onClick={() => handleArrayToggle('additionalExperiences', experience.value)}
              ...
              >
                <div className="flex items-start space-x-3">
                  <div className={`p-2 rounded-lg ${isSelected ? 'bg-white' : 'bg-gray-100'}`}>
                    <Icon size={20} className={getIconColor(experience.color, isSelected)} />
                  </div>
                  <div className="flex-1">
                    <h3 className="font-medium text-gray-800 mb-1">{experience.label}</h3>
      ...
      >
        <p className="text-sm text-amber-800">
          💡 <strong>Tip:</strong> These experiences can be organized as welcome activities, 
          day-after adventures, or extended celebration options for your guests.
        </p>
      </motion.div>
    </motion.div>
  );
